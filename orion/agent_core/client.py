...
def _get_optimized_http_limits() -> httpx.Limits:
    ...
    return httpx.Limits(max_connections=100, max_keepalive_connections=20, keepalive_expiry=30.0)


def _get_optimized_http_timeout() -> httpx.Timeout:
    """Get optimized HTTP timeout settings."""
    return httpx.Timeout(connect=10.0, read=60.0, write=10.0, pool=5.0)
...
class ClientPool:
    ...
    async def get_async_client(self, config: LLMConfig) -> AsyncOpenAI:
        ...
        async with self._async_lock:
            ...
            self._async_client_refs[client_key] = weakref.ref(
                client, lambda ref: self._cleanup_async_client(client_key)
            )
    ...
    def get_sync_client(self, config: LLMConfig) -> OpenAI:
        ...
        with self._sync_lock:
            ...
            self._sync_client_refs[client_key] = weakref.ref(client, lambda ref: self._cleanup_sync_client(client_key))
