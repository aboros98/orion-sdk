...
async def enhance_description_with_llm(func_name: str, description: str) -> str:
    ...
    description_enhancer = build_async_agent(
    ...
    enhanced_description = await description_enhancer(
...
async def function_to_schema(
    func: Callable, enhance_description: bool = False, func_name: Optional[str] = None, needs_memory: bool = False
) -> dict:
    """
    ...
    if hasattr(original_func, "__closure__") and original_func.__closure__:
        for cell in original_func.__closure__:
            if hasattr(cell.cell_contents, "__name__") and cell.cell_contents.__name__ != original_func.__name__:
    ...
    if hasattr(original_func, "system_prompt"):
        ...
        for param in signature.parameters.values():
            try:
                ...
                logger.error(f"Unknown type annotation {param.annotation} for parameter {param.name}: {e}")
